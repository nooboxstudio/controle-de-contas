...
<tbody>
<?php
$query_venc_cont = "SELECT * from conta join condominios on conta.id_cond = condominios.id_cond WHERE vencimento < curdate() AND status=1";
...
<td><?php echo $venc_contas->nome_cond ?></td>
<td>
<?php
...
$data_final = date("Y-m-d", strtotime(str_replace("/", "-", $data_inicial)."+6 days"));
?>
<?php
$originalDate_inicial = $data_inicial;
$newDate_inicial = date("d/m/Y", strtotime($originalDate_inicial));

$originalDate_final = $data_final;
$newDate_final = date("d/m/Y", strtotime($originalDate_final));

?>

<th width="33%">Vencimento de: <?php echo $newDate_inicial?> à <?php echo $newDate_final?></th>
<th width="19%">Condominio</th>
<th width="8%">Data</th>
<th width="40%" align="center" style="text-align:center">Ações</th>
</tr>
...
<tbody>
<?php
$query_prox_cont = "SELECT * from conta join condominios on conta.id_cond = condominios.id_cond WHERE vencimento >= curdate() AND vencimento <= '$data_final' AND status=1 ORDER BY vencimento ASC";
